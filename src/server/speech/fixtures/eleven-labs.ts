...
function handleStt(ws: WebSocket, options: FixtureOptions): void {
  ...
  ws.on('message', (data) => {
    ...
    if (msg.message_type !== 'input_audio_chunk' || typeof msg.commit !== 'boolean' ||
        typeof msg.audio_base_64 !== 'string' || msg.sample_rate !== 16000) {
      send(ws, { message_type: 'input_error', error: 'Invalid STT input contract.' });
      return;
    }
    if (msg.audio_base_64.length > 0) {
      words.push(Buffer.from(msg.audio_base_64, 'base64').toString('utf8'));
      if (options.sttPartials) send(ws, { message_type: 'partial_transcript', text: words.join(' ') });
    }
    if (msg.commit) {
      if (options.sttNoFinal) return;
      if (options.sttErrorOnCommit) {
        send(ws, { message_type: 'transcriber_error', error: options.sttErrorOnCommit });
        return;
      }
      send(ws, { message_type: 'committed_transcript', text: words.join(' ') });
      words.length = 0;
    }
  });
}
