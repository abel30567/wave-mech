...
export function sttCommitFrame(): string {
  return JSON.stringify({ type: 'input_audio_chunk', commit: true });
}
...
export function parseSttEvent(raw: unknown): SttEvent {
  ...
  if (type === 'error' || typeof msg.error === 'string') {
    return { kind: 'error', message: asString(msg.message) || asString(msg.error) || 'stt_error' };
  }

  const text = asString(msg.text ?? msg.transcript);
  const isFinal = msg.is_final === true || msg.final === true;

  if (type === 'final_transcript' || (type === 'transcript' && isFinal) || (isFinal && 'text' in msg)) {
    return { kind: 'final', text };
  }
  if (
    type === 'partial_transcript' ||
    type === 'interim_transcript' ||
    (type === 'transcript' && !isFinal)
  ) {
...
/** Empty text with flush signals end-of-input and requests the final audio. */
