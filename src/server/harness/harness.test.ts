...
interface Harness {
  ...
  close(): Promise<void>;
  readonly diagnostic: import('../../shared/diagnostics.js').DiagnosticLog;
}
...
describe('createHarness — model validation', () => {
  ...
  it('records model in diagnostics snapshot', async () => {
    const { session } = build('basic', { MODEL: 'claude-opus-4-6' });
    await session.start();
    await vi.waitFor(() => {
      expect(session.diagnostic.snapshot().model).toBe('claude-opus-4-6');
    });
  });
});
...
describe('createHarness — NDJSON fixture regressions', () => {
  ...
  it('classifies service failure despite is_error:false integration JSON as failed/integration_error', async () => {
    ...
    expect(terminal?.reason).toBe('integration_error');
  ...
  it('never emits raw arguments/results/errors/tokens in diagnostics for secret payloads', async () => {
    ...
    await built.session.start();
    await built.session.send('go');
    const snap = built.session.diagnostic.snapshot();
    const serialized = JSON.stringify(snap);
    expect(serialized).not.toContain('sk-ant-XXXX');
    expect(serialized).not.toContain('secret-token-value');
    const entry = snap.entries.find((e) => e.callId === 'sec-1');
  ...
  it('settles unresolved active tool attempts on exit/cancellation', async () => {
    const built = build('crash');
    await built.session.start();
    await expect(built.session.send('go')).rejects.toThrow(/exited/i);
    const snap = built.session.diagnostic.snapshot();
    const cancelled = snap.entries.filter((e) => e.status === 'cancelled');
    // crash scenario starts a message but exits before tool completion — no tool_use blocks
    // so there should be no cancelled tools (the message_start has no tool blocks)
    // but the diagnostic should still be a valid empty snapshot
    expect(snap.owner).toBe('harness');
    expect(snap.entries).toBeInstanceOf(Array);
  });

  it('records tool outcomes in the diagnostic snapshot', async () => {
    const built = build('basic');
    await built.session.start();
    await built.session.send('go');
    const snap = built.session.diagnostic.snapshot();
    expect(snap.entries).toHaveLength(1);
    expect(snap.entries[0]).toMatchObject({ callId: 'tool-1', name: 'Read', status: 'done' });
    expect(snap.entries[0].durationMs).toBeTypeOf('number');
  });

  it('does not produce duplicate terminal tool events on normal completion', async () => {
    const built = build('basic');
    await built.session.start();
    await built.session.send('go');
    const toolEvents = built.tools() as Array<Extract<HarnessEvent, { type: 'tool' }>>;
