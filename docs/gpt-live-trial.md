...
Since 2026-09-16 the trial keeps the same bounded `DiagnosticLog` as default mode. Copy diagnostics produces the shared report (format 2): trial header with session limit, start/deadline/end timestamps, the app's stop cause (`user_ended`, `deadline`, `budget_exceeded`, `provider_closed`, …) separate from the provider's closure reason, an unfinished-task flag, opaque delegation records (id, offset, sent/settled timing, status), a timestamped transcript, and `[server:N]` lines carrying tool ids/names/status/duration/reason, backend result timing and turn count, commentary delivery, provider/closure events. Task text, answers, tool payloads and secrets never enter the structured records. The panel shows a countdown to the configured limit and an explicit notice when a session ended at the deadline with a backend task still running.

The session ceiling is `WAVE_GPT_LIVE_MAX_SESSION_SECONDS` (default and maximum 300). The initial 90-second connectivity limit on the installed service was raised to the full five minutes on 2026-09-16; each session still reserves its full duration ($0.25) against the cumulative $5 allowance.
